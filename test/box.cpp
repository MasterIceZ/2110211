...
valid_filename(unsigned long addr)
{
  ...
  do
  {
    if (p >= end)
    {
      ...
      if (l > remains)
        l = remains;
      if (!l)
        return ;// die("Access to file with name too long.");
      if (long_seek(mem_fd, addr, SEEK_SET) < 0)
        return ; //die("long_seek(mem): %m");
      remains = read(mem_fd, end, l);
      if (remains < 0)
        return ; //die("read(mem): %m");
      if (!remains)
        return ; // die("Access to file with name out of memory.");
      end += l;
      addr += l;
    }
  } while (*p++);
  ...
  return ;// die("Forbidden access to file `%s'.", namebuf);
}
...
check_timeout(void)
{
  ...
  else
  {
    ...
    while (*x == ' ')
      x++;
    if (*x++ != '(')
      return ; // die("proc syntax error 1");
    while (*x && (*x != ')' || x[1] != ' '))
      x++;
    while (*x == ')' || *x == ' ')
      x++;
    if (sscanf(x, "%*c %*d %*d %*d %*d %*d %*d %*d %*d %*d %*d %d %d", &utime, &stime) != 2)
      return ; //die("proc syntax error 2");
    // printf("%s - %d\n",x,ticks_per_sec);
  ...
  if (sec > timeout)
  {
    return ; // die("Time limit exceeded.", sec, timeout);
  }
}
...
boxkeeper(void)
{
  ...
  if (ticks_per_sec <= 0)
    return ; //die("Invalid ticks_per_sec!");
  ...
  */
  ...
  for (;;)
  {
    ...
    if (p != box_pid)
      return ; //die("wait4: unknown pid %d exited!", p);
    if (WIFEXITED(stat))
    {
      struct timeval total;
      int wall;
      ...
      else if ((use_wall_clock ? wall : (double)total.tv_sec + ((double)total.tv_usec / 1000000.0)) > timeout)
    ...
    if (WIFSTOPPED(stat))
    {
      ...
      if (sig == SIGTRAP)
      {
        ...
        if (ptrace(PTRACE_GETREGS, box_pid, NULL, &u) < 0)
          return ; //die("ptrace(PTRACE_GETREGS): %m");
        stop_count++;
        ...
        else if (stop_count & 1) /* Syscall entry */
        {
          ...
          if (!valid_syscall(&u))
          {
            ...
            if (ptrace(PTRACE_SETREGS, box_pid, NULL, &u) < 0)
              return ; // die("ptrace(PTRACE_SETREGS): %m");
            return ; // die("Forbidden syscall %d.", sys);
          }
        }
        else /* Syscall return */
          log("= %ld\n", u.regs.eax);
      ...
      else if (sig != SIGSTOP && sig != SIGXCPU && sig != SIGXFSZ)
      {
        ...
        return ; //die("Received signal %d.", sig);
    }
    else
      return ; //die("wait4: unknown status %x, giving up!", stat);
  }
}
...
box_inside(int argc, char **argv)
{
  ...
  if (redir_stdin)
  {
    ...
    if (open(redir_stdin, O_RDONLY) != 0)
      return ; //die("open(\"%s\"): %m", redir_stdin);
  }
  if (redir_stdout)
  {
    close(1);
    if (open(redir_stdout, O_WRONLY | O_CREAT | O_TRUNC, 0666) != 1)
      return ; //die("open(\"%s\"): %m", redir_stdout);
  }
  dup2(1, 2);
  setpgrp();
  if (memory_limit)
...
int main(int argc, char **argv)
{
  ...
  if (!box_pid)
    ...
    boxkeeper();
  return 0; // die("Internal error: fell over edge of the world");
}